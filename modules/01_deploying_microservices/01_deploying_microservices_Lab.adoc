...
NOTE: In the next step we will created Docker images for our service. We make use of Podman to create the Docker images. Podman manage pods, containers, and container images. Podman is theoretically a drop-in replacement for the `docker` command-line tool. Simply put: `alias docker=podman`. For more details on Podman, see the website: https://podman.io.
