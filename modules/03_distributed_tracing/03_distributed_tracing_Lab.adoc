...
----
+
NOTE: Don't make any changes to this code. We are only reviewing the code.
...
. Since we are using Java there is a no-code approach that simply requires a few lines added to your Maven POM file and setting some environment variables. These lines are in the `pom.xml` file. This will trigger the injection of the Jaeger Tracer Resolver.
...
* Docker file location: `~/lab/rhte-msa-and-service-mesh/gateway/java/springboot/Docker`
+
----
