...
----
+ 
* Now you should see 7 requests, 70% going to `v1`. You should see 3 requests, 30% going to `v2`. This process can be continued (and automated), slowly migrating
...
* File: src/com/redhat/developer/demos/gateway/GatewayController.java
+
----
    @RequestMapping("/")
...
. Set all requests to go to `catalog:v1`.
+
----
istioctl replace -f istiofiles/virtual-service-catalog-v1.yml -n $OCP_TUTORIAL_PROJECT --as=system:admin
----

. We can configure Istio to send Safari users to `catalog:v2`. Below is the configuration file.

* File: `virtual-service-safari-catalog-v2.yml`
+
----
apiVersion: networking.istio.io/v1alpha3
kind: VirtualService
...
---
----
* This definition performs a match on http headers for `baggage-user-agent`. It makes use of a regular expression to match on the the string `Safari`. If there is a match then the traffic is routed to `catalog:v2`. Else, all other traffic is routed to `catalog:v1`.

. Set Safari users to use `catalog:v2`
+
----
istioctl replace -f istiofiles/virtual-service-safari-catalog-v2.yml -n $OCP_TUTORIAL_PROJECT --as=system:admin
----

. Test the app using a Safari user agent.
+
----
