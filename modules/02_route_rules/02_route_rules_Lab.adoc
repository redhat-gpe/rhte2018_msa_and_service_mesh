...
. Below is an istio configuration file to route 75% of traffic to `v1` and 25% of traffic to `v2`. 
...
** This definition allows you to configure a percentage of traffic and direct it to a specific version of the `catalog` service. In this case, 75% of traffic _(weight)_ for the catalog service will always go to pods matching the labels version: `v1`. The remaining 25% of traffic will be routed to `v2` of the `catalog` service.
